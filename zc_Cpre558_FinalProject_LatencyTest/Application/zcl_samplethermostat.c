...
//    strcat(lineFormat1, "["CUI_COLOR_YELLOW"Sent Value"CUI_COLOR_RESET"] 0x%04x ");
    strcat(lineFormat1, "["CUI_COLOR_YELLOW"Received Value"CUI_COLOR_RESET"] 0x%02x");
//    strcat(lineFormat1, "["CUI_COLOR_YELLOW"Received Value"CUI_COLOR_RESET"] 0x%04x");
//    strcat(lineFormat1, "["CUI_COLOR_YELLOW"Received Value"CUI_COLOR_RESET"] 0x%08x");
//    strcat(lineFormat2, "["CUI_COLOR_YELLOW"Avg Response Time"CUI_COLOR_RESET"] %d s : %d ms");

//    for (i = 0; i < timer_data_size; i++) { timer_average += timer_data[i]; }
//    timer_average /= timer_data_size;

    CUI_statusLinePrintf(gCuiHandle, gSampleThermostatInfoLine1, lineFormat1, data_recieve);
//    CUI_statusLinePrintf(gCuiHandle, gSampleThermostatInfoLine2, lineFormat2, (int)timer_average, (int)(timer_average * 1000));
}
